...
function addCustomerName() {
    ...
    headone.textContent = customerName;
    // <h2 id="introheader" class="tm-section-title mb-5 text-uppercase tm-color-primary"></h2> <!--todo: Add Customer Name-->
    let headtwo = document.querySelector("#introheader")
    headtwo.textContent = customerName;
};

function toggleColors() {
    // <!-- todo: Change all classes named 'tm-white-rect' to 'tm-blue-rect' -->
    let tmrect = document.querySelectorAll(".tm-white-rect")
    for (i = 0; i < tmrect.length; ++i) {
        tmrect[i].classList.add('tm-blue-rect');
        tmrect[i].classList.remove('tm-white-rect');
    }
};

function hideElement() {
    //<ul class="nav flex-row tm-social-links">  <!--todo: Hide social links-->
    let hidi = document.querySelectorAll('.tm-social-links')
    hidi.style.visibility = "none";
};
